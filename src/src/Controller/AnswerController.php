...
class AnswerController extends AbstractController
{
    ...
    #[Route('/new/{id}/{answerFormData}', name: 'app_answer_new', methods: ['GET', 'POST'])]
    public function new($id,$answerFormData,Request $request, AnswerRepository $answerRepository, QuestionRepository $questionRepository): Response
    {
        $answer = new Answer();
        $question = $questionRepository->find($id);

        $answer->setAnswer($answerFormData)
                ->setQuestion($question);

        $questionRepository->save($answer, true);
